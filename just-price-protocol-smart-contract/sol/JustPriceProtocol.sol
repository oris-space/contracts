...
contract JustPriceProtocol is SafeMath {
  ...
  function vote (address newK1) public {
    updateStage ();
    ...
    if (votesCount > 0) {
      ...
      if (newK1 != oldK1) {
        ...
        votes [msg.sender] = newK1;

        if (newK1 != address (0)) {
          voteResults [newK1] = safeAdd (voteResults [newK1], votesCount);
          Vote (msg.sender, newK1, votesCount);

          if (safeMul (voteResults [newK1], REQUIRED_VOTES_DENOMINATOR) >=
